...
LevelMod = {
        # 70 : dict(main=364, div=900, attr=327, vital=292),
        70 : dict(main=364, div=900),
        90 : dict(main=400, div=1900),
        }
...
def ExpDmgSum(attr, dh, crit, det, sks, tncpt, haste, levelmod=70, GCD=250):
    ...
    result = attr
    result *= calc.dh_exDmg(dh, 2)
    result *= calc.crit_exDmg(crit, 5)
...
class StatsCalc:
    ...
    def calc_base(self, mul, stat) -> float:
        ...
        return calc
    def calc_base292(self, mul, stat) -> float:
        calc = (mul*(stat-292)/LevelMod[self.levelmod]['div'])
        return calc
    ...
    def det_exDmg(self, stat) -> float:
        calc = int(1000 + self.calc_base292(140, stat)) / 1000
        # print('det {}', calc)
        return calc
